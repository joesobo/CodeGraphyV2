...
const receiveMessages = async (webview: vscode.Webview) => {
	...
	webview.onDidReceiveMessage(async (message) => {
		...
		case 'openFile':
			await vscode.workspace.openTextDocument(vscode.Uri.file(message.text)).then(async (doc) => {
				await vscode.window.showTextDocument(doc)
			})

			await getGraphData(webview, { nodeSize: saveNodeSize, interactionConnections: saveInteractionConnections, nodeDepth: saveNodeDepth })
...
const getGraphData = async (webview: vscode.Webview, message: {nodeSize: string, interactionConnections: string, nodeDepth: number}) => {
	...
	if (message.interactionConnections === 'Interaction') {
		...
		processedData = processData(files, message.nodeSize, connections, currentOpenFile, message.nodeDepth)

	} else {
		connections = fetchDirectoryConnections(files, dirs)
		processedData = processData(files, message.nodeSize, connections, currentOpenFile, message.nodeDepth, dirs)
	}
