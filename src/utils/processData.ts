...
export const processData = (
	...
	if (nodeDepth > 0 && openFile) {
		const queue: {file: string, depth: number}[] = [{ file: openFile, depth: 0 }]
		...
		while (queue.length > 0) {
			...
			if (currentFile) {
				...
				const fileIndex = files.findIndex((file) => file.name === currentFile.name)
				...
				connections.forEach((connection) => {
					...
					if (sourceIndex === fileIndex || targetIndex === fileIndex) {
						...
						if (!touchedIndices.includes(nonSelectedIndex) && current.depth < nodeDepth) {
							queue.push({ file: files[nonSelectedIndex].name, depth: current.depth + 1 })

							const s = nodes.indexOf(nodes.find((node) => node.id === fileIndex)!)
